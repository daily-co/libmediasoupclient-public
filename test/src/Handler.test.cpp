...
TEST_CASE("SendHandler", "[Handler][SendHandler]")
{
	...
	SECTION("sendHandler.Send() succeeds if a track is provided")
	{
		...
		REQUIRE_NOTHROW(sendResult = sendHandler.Send(track.release(), nullptr, nullptr, nullptr));
	...
	{
		REQUIRE_NOTHROW(sendHandler.Send(track.release(), nullptr, nullptr, nullptr));
	}
	...
	SECTION("sendHandler.ReplaceTrack() succeeds if a new track is provided")
	{
		...
		REQUIRE_NOTHROW(sendHandler.ReplaceTrack(localId, newTrack.release()));
